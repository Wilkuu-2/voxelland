            ...
            Some(stream) => {
                        ...
                        Ok(numbytes) => {
                            ...
                            if numbytes > 0 {
                                let mut message: Message =
                                    match bincode::deserialize(&buffer[..numbytes]) {
                                        Ok(m) => m,
                                        Err(_) => {
                ...
                match message.message_type {
                    ...
                    MessageType::RequestUdm => {
                        ...
                        let udmmsg =
                            Message::new(MessageType::Udm, Vec3::ZERO, 0.0, buffer.len() as u32);

                        {
                            let mut mystream = stream.lock().unwrap();
                            mystream.set_nonblocking(false);
                            mystream
                                .write_all(&bincode::serialize(&udmmsg).unwrap())
                                .unwrap();
                    ...
                    MessageType::RequestSeed => {
...

                        {
                            let mut mystream = stream.lock().unwrap();
                            mystream
                                .write_all(&bincode::serialize(&seedmsg).unwrap())
                                .unwrap();
                        }
                    }
                    MessageType::ChestInvUpdate => {
                        ...
                        match slotindextype {
                            SlotIndexType::ChestSlot(e) => {
                                let mut chestinv =
                                    chest_reg.entry(currchest).or_insert(ChestInventory {
                                        dirty: false,
                                        inv: [(0, 0); 20],
                                    });
                                ...
                                let currseed =
                                    (*csys.read().unwrap().currentseed.read().unwrap()).clone();

                                queued_sql.push(QueuedSqlType::ChestInventoryUpdate(
                                    currchest,
                                    chestinv.inv.clone(),
                                    currseed,
                                ));
                            ...
                            SlotIndexType::InvSlot(e) => {
                                ...
                                queued_sql.push(QueuedSqlType::InventoryInventoryUpdate(
                                    client_id,
                                    clientlock.get(&client_id).unwrap().inv.inv,
                                ));
                            }
                            SlotIndexType::None => {}
                        }
                    ...
                    MessageType::PlayerUpdate => {
...

                        {
                            ...
                            if client.saveposcounter > 10 {
                                ...
                                queued_sql.push(QueuedSqlType::PlayerPositionUpdate(
                                    client_id,
                        ...
                        let mut timeupdate = Message::new(
                            MessageType::TimeUpdate,
                            Vec3::ZERO,
                            unsafe { WEATHERTYPE },
                            unsafe { SONGINDEX } as u32,
                        );
                        ...
                        if sendmobs {
                            let mobmsgs = {
                                knowncams
                                    .insert(client_id, Vec3::new(message.x, message.y, message.z));

                                let nlock = nsmes.lock().unwrap();
                                let mobmsgs: Vec<Message> = nlock
                                    .iter()
                                    .map(|nsme| {
                                        let mut mobmsg = Message::new(
                            ...
                            for chunk in mobmsgs.chunks(server_types::MOB_BATCH_SIZE) {
                                let mut mobmsg =
                                    Message::new(MessageType::MobUpdateBatch, Vec3::ZERO, 0.0, 0);
                    ...
                    MessageType::RequestPt => {
                        ...
                        {
                            let ptmsg =
                                Message::new(MessageType::Pt, Vec3::ZERO, 0.0, currpt as u32);
                            let mut mystream = stream.lock().unwrap();
...
fn main() {
    ...
    unsafe {
        PACKET_SIZE = bincode::serialized_size(&Message::new(
            MessageType::RequestSeed,
            ...
            match {
                match sql {
                    ...
                    QueuedSqlType::ChestInventoryUpdate(key, inv, seed) => {
                        ...
                        stmt.execute(params![key.x, key.y, key.z, false, inv_bin])
                    }
    ...
    loop {
        match listener.accept() {
            Ok((stream, _)) => {
                ...
                while !gotid && retries < 100 {
                    let mut buffer = Vec::new();
                    buffer.resize(
                        bincode::serialized_size(&Message::new(
                            MessageType::BlockSet,
                            Vec3::ZERO,
                            0.0,
                            0,
                    ...
                    match stream.lock().unwrap().read_exact(&mut buffer) {
                        Ok(_bytes) => match bincode::deserialize::<Message>(&buffer) {
                            Ok(comm) => {
                                if comm.message_type == MessageType::TellYouMyID {
                                    ...
                                    println!(
                                        "Received greeting but it was the wrong messagetype {}",
                                        comm.message_type
                                    );
                                }
                            }
        ...
        *nblock = nsme
        ...
        if mobspawnqueued.load(std::sync::atomic::Ordering::Relaxed) {
            ...
            if true {
                //chunksys.read().unwrap().planet_type == 1 {
                let mut rng = StdRng::from_entropy();
                let mut gamewrite = gamearc.write().unwrap();
                gamewrite.create_non_static_model_entity(
                    0,
