...
impl CollCage {
    ...
    pub fn get_smoothed_floor_y(&mut self, actualpos: Vec3) -> f32 {
        ...
        if self.solid.contains(&Side::LEFTBOTTOM) {
            neg_x_height = 1.0;
        }
        if self.solid.contains(&Side::RIGHTBOTTOM) {
            pos_x_height = 1.0;
        }
        if self.solid.contains(&Side::BACKBOTTOM) {
            neg_z_height = 1.0;
        }
        if self.solid.contains(&Side::FRONTBOTTOM) {
            pos_z_height = 1.0;
        }
        ...
        let dominant_axis = if frac_x.abs() > frac_z.abs() {
            lerped_height_x
        } else {
            lerped_height_z
        };
