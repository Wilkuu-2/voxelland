...
use crate::{
    collisioncage::{CollCage, Side},
    game::*,
    modelentity::{AggroTarget, ModelEntity},
    vec,
};
use dashmap::DashMap;
...
fn load_textures(images: &[gltf::image::Data]) -> Vec<GLuint> {
    images
        .iter()
        .map(|image_data| {
            let img = image::load_from_memory(&image_data.pixels)
                .expect("Failed to decode image")
                .to_rgba8();
            let dimensions = img.dimensions();
...
fn rasterize_triangle(triangle: [Vec3; 3], collision_map: &DashMap<vec::IVec3, u8>) {
    ...
    let min_z = triangle
        .iter()
        .map(|v| v.z.floor() as i32)
        .min()
        .unwrap_or(0);
    let max_z = triangle
        .iter()
        .map(|v| v.z.ceil() as i32)
        .max()
        .unwrap_or(0);
...
impl Game {
                ...
                for i in (0..indices.len()).step_by(3) {
                        ...
                        (indices.get(i), indices.get(i + 1), indices.get(i + 2))
                    {
                        ...
                        let transformed_triangle = triangle.map(|vertex| {
                            ...
                            let rotated_vertex =
                                (get_rotation_matrix(entity.rot.x, entity.rot.y, entity.rot.z)
                                    * Vec4::new(
                                        scaled_vertex.x,
                                        scaled_vertex.y,
                                        scaled_vertex.z,
                        ...
                        rasterize_triangle(
                            ...
                            &self.chunksys.read().unwrap().justcollisionmap,
                        );
                    }
                }
            }
    ...
    pub fn create_non_static_model_entity(
        &mut self,
        model_index: usize,
        pos: Vec3,
        scale: f32,
        ...
        let mut modent = ModelEntity::new_with_jump_height(
            model_index,
            pos,
            scale,
            rot,
    ...
    pub fn insert_static_model_entity(
        &mut self,
        id: u32,
        model_index: usize,
        pos: Vec3,
        ...
        let mut modent = ModelEntity::new_with_id(
            id,
            model_index,
            pos,
            scale,
    ...
    pub fn insert_player_model_entity(
        &mut self,
        id: Uuid,
        model_index: usize,
        pos: Vec3,
        ...
        let mut modent = ModelEntity::new_with_id(
            0, /*Does not use model entities id system, uses players id system */
            model_index,
            pos,
            scale,
            rot,
                        ...
                        Some(str) => {
                            ...
                            unsafe {
                                AUDIOPLAYER.play(
                                    ...
                                    &model.position,
                                    &model.velocity,
                                    Planets::get_mob_volume(model.model_index),
                                );
                            }
                        }
                        None => {}
    ...
    pub fn take_damage(&mut self, amount: u8) {
        ...
        if newamount <= 0 {
            ...
            for i in 0..5 {
                ...
                self.drops
                    .add_drop(campos + Vec3::new(0.0, 2.0, 0.0), inv.inv[i].0, amt);
            }
    ...
    pub fn update_non_static_model_entities(&mut self) {
        ...
        for mut model in self.non_static_model_entities.iter_mut() {
            ...
            if !model.grounded && !model.jumping_up {
                model.time_falling_scalar =
                    (model.time_falling_scalar + self.delta_time * 5.0).min(3.0);
            } else {
            ...
            if !model.grounded && !model.jumping_up {
                model.velocity += Vec3::new(
                    0.0,
                    -GRAV * model.time_falling_scalar * self.delta_time,
                    0.0,
                );
            }
    ...
    pub fn draw_models(&self) {
        unsafe {
            ...
            let mvp_loc =
                gl::GetUniformLocation(self.modelshader.shader_id, b"mvp\0".as_ptr() as *const i8);
            ...
            let nsme = self
                ...
                .collect::<Vec<_>>();
            let pme = self
                .player_model_entities
                .iter()
                .map(|e| e)
                .collect::<Vec<_>>();

            for modelt in self
                .static_model_entities
                .iter()
                .map(ModelEntityType::Static)
                .chain(
                    nsme.iter()
                        .map(|arg0| ModelEntityType::NonStatic(arg0.value())),
                )
                .chain(
                    pme.iter()
                        .map(|arg0| ModelEntityType::NonStatic(arg0.value())),
                ...
                if index < self.gltf_vaos.len() && index < self.gltf_textures.len() {
                    ...
                    for (ind, vaoset) in vaosetset.iter().enumerate() {
                        ...
                        for (ii, vao) in vaoset.iter().enumerate() {
                            ...
                            let fogcol = Planets::get_fog_col(
                                self.chunksys.read().unwrap().planet_type as u32,
                            );
    ...
    pub fn load_model(&mut self, path: &'static str) {
        ...
        self.gltf_models
            .push((document.clone(), buffers.clone(), images.clone()));
            ...
            for channel in animation.channels() {
                ...
                let outputs: Vec<Vec<f32>> = match reader.read_outputs().unwrap() {
                    ReadOutputs::Translations(translations) => {
                        translations.map(|v| v.to_vec()).collect()
                    }
                    ReadOutputs::Rotations(rotations) => {
                        rotations.into_f32().map(|v| v.to_vec()).collect()
                    }
    ...
    fn collect_indices(data: ReadIndices) -> Vec<u32> {
        match data {
            ReadIndices::U8(iter) => iter.map(|index| index as u32).collect(),
            ReadIndices::U16(iter) => iter.map(|index| index as u32).collect(),
    ...
    pub fn create_model_vbos(&mut self) {
        for (index, (document, buffers, _images)) in self.gltf_models.iter().enumerate() {
            ...
            let textures =
                load_document_textures(&document, &buffers, self.gltf_paths[index].as_str());
