...
impl BlockOverlay {
    pub fn new(texture: GLuint) -> BlockOverlay {
        ...
        let faces: [f32; 144] = [
            -0.55, -0.5, 0.5, 0.0, -0.55, -0.5, -0.5, 1.0, -0.55, 0.5, -0.5, 2.0, -0.55, 0.5, -0.5,
            3.0, -0.55, 0.5, 0.5, 4.0, -0.55, -0.5, 0.5, 5.0, 0.55, -0.5, -0.5, 0.0, 0.55, -0.5,
            0.5, 1.0, 0.55, 0.5, 0.5, 2.0, 0.55, 0.5, 0.5, 3.0, 0.55, 0.5, -0.5, 4.0, 0.55, -0.5,
            -0.5, 5.0, 0.5, -0.5, 0.55, 0.0, -0.5, -0.5, 0.55, 1.0, -0.5, 0.5, 0.55, 2.0, -0.5,
            0.5, 0.55, 3.0, 0.5, 0.5, 0.55, 4.0, 0.5, -0.5, 0.55, 5.0, -0.5, -0.5, -0.55, 0.0, 0.5,
            -0.5, -0.55, 1.0, 0.5, 0.5, -0.55, 2.0, 0.5, 0.5, -0.55, 3.0, -0.5, 0.5, -0.55, 4.0,
            -0.5, -0.5, -0.55, 5.0, -0.5, 0.55, -0.5, 0.0, 0.5, 0.55, -0.5, 1.0, 0.5, 0.55, 0.5,
            2.0, 0.5, 0.55, 0.5, 3.0, -0.5, 0.55, 0.5, 4.0, -0.5, 0.55, -0.5, 5.0, 0.5, -0.55,
            -0.5, 0.0, -0.5, -0.55, -0.5, 1.0, -0.5, -0.55, 0.5, 2.0, -0.5, -0.55, 0.5, 3.0, 0.5,
            -0.55, 0.5, 4.0, 0.5, -0.55, -0.5, 5.0,
        ];
        ...
        unsafe {
            ...
            gl::NamedBufferData(
                vbo,
                (faces.len() * std::mem::size_of::<f32>()) as GLsizeiptr,
    ...
    pub fn draw_at(&self, pos: Vec3, breakprogress: i8, mvp: &Mat4, walkbob: f32) {
        unsafe {
            ...
            let bp_loc = gl::GetUniformLocation(
                self.shader.shader_id,
                b"breakPhase\0".as_ptr() as *const i8,
            );

            gl::Uniform1f(bp_loc, breakprogress as f32);

            let bp_loc = gl::GetUniformLocation(
                self.shader.shader_id,
                b"blockPosition\0".as_ptr() as *const i8,
            );

            gl::Uniform3f(bp_loc, pos.x, pos.y, pos.z);
